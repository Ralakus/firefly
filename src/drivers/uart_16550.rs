...
impl Uart16550<Mmio<u8>> {
    /// # Safety
    /// Only create one UART MMIO instance per memory address.
    /// Creates UART device at memory address.
    ...
    T::Value: From<u8> + TryInto<u8>,
{
    ...
    /// Reads in a single byte from the UART device.
    ...
    /// Writes a single byte to UART device.
    pub fn send(&mut self, data: u8) {
        while !self.line_sts().contains(LineStsFlags::OUTPUT_EMPTY) {}
        unsafe { self.data.write(data.into()) }
    }

    /// Writes a byte slice to UART device.
