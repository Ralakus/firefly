...
kernel = target/riscv64gc-unknown-none-elf/$(build_mode)/firefly
...
clean:
	-rm -R target hdd.img Cargo.lock dep_graph.png firefly.bin 

bin: $(kernel)
	$(OBJCOPY) -O binary $(kernel) firefly.bin 